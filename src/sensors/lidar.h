...
struct Lidar
{
		...
		: cloud(new pcl::PointCloud<pcl::PointXYZ>()), position(0,0,2.6)
	{
		...
		minDistance = 4;// Distance of rays from roof changed from 0 to 5m
		maxDistance = 50;
		resoultion = 0.2;
		// TODO:: set sderr to 0.2 to get more interesting pcd files
		sderr = 0.2;// Set the noise of sensor from 0m to 0.2m
		cars = setCars;
		groundSlope = setGroundSlope;

		// TODO:: increase number of layers to 8 to get higher resoultion pcd
		int numLayers = 32;// Layers changed from 3 to 8
		...
		double horizontalAngleInc = pi/64;// Changed from pi/6 to pi/64
